import { Typography } from 'antd'

import MathJax from '@/components/assets/mathjax'
import Carousel from '@/components/assets/carousel'

import PostLayout, { Ref } from '../layout'

const key = '1-LinearElasticity'
const title = 'Linear elasticity problems'
const description = 'Validation tests, linear elasticity'
const date = '2023-02-14'
const image = '/img/blog/3-linearElasticity.jpg'
const keywords = ['Mechanics', 'Validation', 'Theory']
const author = {
  name: 'Houssam Houssein',
  url: 'https://github.com/houssamh'
}

const references = [
  {
    code: '1',
    author: 'Reddy, J. N.',
    date: '2013',
    label: 'An introduction to continuum mechanics.',
    journal: 'Cambridge university press.'
  }
]

/**
 * Post
 * @returns Post
 */
const Post: React.FunctionComponent = () => {
  return (
    <PostLayout
      title={title}
      date={date}
      image={image}
      keywords={keywords}
      author={author}
      version={'1.2.6'}
      references={references}
    >
      <section>
        <Typography.Title level={3}>Theory</Typography.Title>
        <p>
          Let <MathJax.Inline text={String.raw`\Omega`} /> be a domain of{' '}
          <MathJax.Inline text={String.raw`\mathbb{R}^{d}`} /> representing the
          body, with <MathJax.Inline text={String.raw`d\in\{2,3\}`} />. The
          domain <MathJax.Inline text={String.raw`\Omega`} /> is bounded by{' '}
          <MathJax.Inline text={String.raw`\Gamma = \Gamma_D \cup \Gamma_N`} />.
        </p>
        <p>
          Let <MathJax.Inline text={'u'} /> be the displacement field. The
          linear elasticity problem is the set of the following equations
          <MathJax.Formula
            text={String.raw`         
            \begin{align}
                -\text{div}(\sigma) &= f &\text{on }\Omega\\
                u &= u_D&\text{on }\Gamma_D\\
                \sigma\cdot n &= \sigma_N&\text{on }\Gamma_N
            \end{align}
          `}
          />
          in addition to the Hook&apos;s law{' '}
          <MathJax.Inline
            text={String.raw`\sigma= \lambda tr(\epsilon)I + 2\mu\epsilon`}
          />
          , where <MathJax.Inline text={String.raw`\sigma`} />,{' '}
          <MathJax.Inline text={String.raw`\epsilon`} /> are respectively the
          stress and strain tensor.{' '}
          <MathJax.Inline text={String.raw`\lambda`} /> and{' '}
          <MathJax.Inline text={String.raw`\mu`} /> are the Lamé coefficients.
          For more details, see <Ref code="1" />
        </p>
      </section>

      <section>
        <Typography.Title level={3}>
          Validation test: Compression of an elastic cube
        </Typography.Title>

        <Typography.Title level={4}>Problem</Typography.Title>
        <p>
          Consider an elastic cube{' '}
          <MathJax.Inline
            text={String.raw`[0 \, m \, , 1 \, m]\times[0 \, m \, , 1 \, m]\times[0 \, m \, , 1 \, m]`}
          />{' '}
          with Young&apos;s modulus{' '}
          <MathJax.Inline text={String.raw`E=2.1e11 \, Pa`} /> and
          Poisson&apos;s ratio <MathJax.Inline text={String.raw`\nu= 0.3`} />.
          The plan <MathJax.Inline text={String.raw`\{ z=0 \}`} /> of the cube
          is fixed in the direction <MathJax.Inline text={'z'} />, the plan{' '}
          <MathJax.Inline text={String.raw`\{ x=1 \}`} /> of the cube is fixed
          in the direction <MathJax.Inline text={'x'} /> and finally the plan{' '}
          <MathJax.Inline text={String.raw`\{ y=0 \}`} /> of the cube is fixed
          in the direction <MathJax.Inline text={'y'} />.
        </p>
        <p>
          A surface load{' '}
          <MathJax.Inline
            text={String.raw`f=(0 \, Pa \, ,0 \, Pa \, ,-1e8 \, Pa)`}
          />{' '}
          is applied on the plan <MathJax.Inline text={String.raw`\{ z=1 \}`} />{' '}
          of the cube.
        </p>
        <p>
          Theoretically all components of the stress tensor are zero except{' '}
          <MathJax.Inline text={String.raw`\sigma_{zz}`} /> which is equal to{' '}
          <MathJax.Inline text={String.raw`\sigma_{zz}=-1e8 \, Pa`} />, so the
          von Mises stress is constant and is equal to{' '}
          <MathJax.Inline text={String.raw`\sigma_{vM}=1e8 \, Pa`} />.
        </p>
      </section>

      <section>
        <Typography.Title level={4}>Simulation and Results</Typography.Title>
        <p>
          Using Tanatloc, we create our geometry with the corresponding mesh
          (see Figures below) and we use P1 linear finite elements. We can also
          see in the Figures below that we obtain the theoretical results,
          independently of the used mesh.
        </p>
        <Carousel
          items={[
            {
              key: 'MeshGeometry',
              src: '/img/blog/3-linearElasticity/GeoMesh.jpg',
              caption: 'Geometry and Mesh'
            },
            {
              key: 'linearElastDisp',
              src: '/img/blog/3-linearElasticity/validation1_displacementResults.jpg',
              caption: 'Displacement result'
            },
            {
              key: 'stressZz',
              src: '/img/blog/3-linearElasticity/validation1_stressZz.jpg',
              caption: 'Stress zz'
            },
            {
              key: 'linearElastvM',
              src: '/img/blog/3-linearElasticity/validation1_vMStress.jpg',
              caption: 'von Mises stress'
            }
          ]}
        />
      </section>
    </PostLayout>
  )
}

const LinearElasticity = {
  default: Post,
  key,
  title,
  description,
  date,
  image,
  keywords,
  author
}

export default LinearElasticity
